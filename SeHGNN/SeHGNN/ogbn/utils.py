...
from torch_sparse import SparseTensor, remove_diag, set_diag
        ...
        for etype in new_g.etypes:
            ...
            for k in list(new_g.nodes[stype].data.keys()):
                if len(k) == hop:
                    ...
                    if (
                        hop == num_hops and dtype != tgt_type and k not in reserve_heads
                    ) or (hop > num_hops and k not in reserve_heads):
                        continue
                    if echo:
                        print(k, etype, current_dst_name)
                    new_g[etype].update_all(
                        fn.copy_u(k, "m"), fn.mean("m", current_dst_name), etype=etype
                    )
...
def clear_hg(new_g, echo=False):
    if echo:
        print("Remove keys left after propagation")
    for ntype in new_g.ntypes:
...
def check_acc(preds_dict, condition, init_labels, train_nid, val_nid, test_nid):
    ...
    for k, v in preds_dict.items():
        ...
        a, b, c = (
            pred[train_nid] == init_labels[train_nid],
    ...
    for idx_1, idx_2 in zip(train_loader, enhance_loader):
        ...
        if scalar is not None:
            with torch.cuda.amp.autocast():
                ...
                L2 = F.cross_entropy(
                    output_att[len(idx_1) :], extra_y, reduction="none"
                )
...
def load_dataset(args):
    if args.dataset == "ogbn-products":
        ...
        assert (
            0
        ), "Only allowed [ogbn-products, ogbn-proteins, ogbn-arxiv, ogbn-papers100M, ogbn-mag]"
...
def load_homo(args):
    ...
    diag_name = f"./data/{args.dataset}_diag.pt"
...
def load_mag(args, symmetric=True):
    ...
    if len(args.extra_embedding):
        ...
        path = os.path.join(args.emb_path, f"{args.extra_embedding}")
        author_emb = torch.load(
            os.path.join(path, "author.pt"), map_location=torch.device("cpu")
        ).float()
        topic_emb = torch.load(
            os.path.join(path, "field_of_study.pt"), map_location=torch.device("cpu")
        ).float()
        institution_emb = torch.load(
            os.path.join(path, "institution.pt"), map_location=torch.device("cpu")
        ).float()
    else:
        author_emb = torch.Tensor(g.num_nodes("author"), args.embed_size).uniform_(
            -0.5, 0.5
        )
        topic_emb = torch.Tensor(
            g.num_nodes("field_of_study"), args.embed_size
        ).uniform_(-0.5, 0.5)
        institution_emb = torch.Tensor(
            g.num_nodes("institution"), args.embed_size
        ).uniform_(-0.5, 0.5)
    ...
    diag_name = f"./data/{args.dataset}_PFP.pt"
    if not os.path.exists(diag_name):
        PF = FP.t()
    ...
    if symmetric:
        diag_name = f"./data/{args.dataset}_PPP_diag.pt"
        if not os.path.exists(diag_name):
            # PP = PP.to_symmetric()
    ...
    diag_name = f"./data/{args.dataset}_PAP_diag.pt"
    if not os.path.exists(diag_name):
