...
def main(args, given_model: Optional[SeHGNN_mag] = None):
    if args.seed > 0:
        set_random_seed(args.seed)
    g, init_labels, num_nodes, n_classes, train_nid, val_nid, test_nid, evaluator = (
        load_dataset(args)
    )
    ...
    if args.dataset in ["ogbn-proteins", "ogbn-products"]:  # homogeneous
        ...
        "ogbn-arxiv",
        "ogbn-papers100M",
    ]:  # single-node-type & double-edge-types
        tgt_type = "P"
        ...
        for hop in range(1, args.num_hops + 1):
            for k in list(g.ndata.keys()):
                if len(k) == hop:
                    g["cite"].update_all(
                        fn.copy_u(k, "msg"), fn.mean("msg", f"m{k}"), etype="cite"
                    )
                    g["cited_by"].update_all(
                        fn.copy_u(k, "msg"), fn.mean("msg", f"t{k}"), etype="cited_by"
                    )
    ...
    checkpt_file = (
        checkpt_folder + f"b={args.b}_use_blb={args.use_blb}_seed={args.seed}"
    )
    print(checkpt_file)
    ...
    for stage in range(args.start_stage, len(args.stages)):
        ...
        ## CREATING BLB SAMPLER HERE ##
        gen = torch.Generator().manual_seed(args.seed)
        idx_perm = torch.randperm(train_node_nums, generator=gen)
        train_idx = torch.arange(train_node_nums)[
            idx_perm[: int(args.b * train_node_nums)]
        ]
        sampler = None

        if args.use_blb:
            gen = gen.manual_seed(args.seed + args.replica_idx)
            idx_perm = torch.randperm(len(train_idx), generator=gen)
            train_idx = train_idx[idx_perm[: int(args.b_rep * len(idx_perm))]]
            sampler = BLBSampler(
                train_idx,
                seed=args.seed,
                bootstrap_size=int(args.b * train_node_nums),
            )
        ...
        if stage > 0:
            ...
            val_acc = evaluator(
                preds[trainval_point:valtest_point],
                labels[trainval_point:valtest_point],
            )
            test_acc = evaluator(
                preds[valtest_point:total_num_nodes],
                labels[valtest_point:total_num_nodes],
            )

            print(
                f"Stage {stage-1} history model:\n\t"
            ...
            val_enhance_offset = torch.where(
                confident_mask[trainval_point:valtest_point]
            )[0]
            test_enhance_offset = torch.where(
                confident_mask[valtest_point:total_num_nodes]
            )[0]
            ...
            print(
                f"Stage: {stage}, threshold {args.threshold}, confident nodes: {len(enhance_nid)} / {total_num_nodes - trainval_point}"
            )
            val_confident_level = (
                predict_prob[val_enhance_nid].argmax(1) == labels[val_enhance_nid]
            ).sum() / len(val_enhance_nid)
            print(
                f"\t\t val confident nodes: {len(val_enhance_nid)} / {valid_node_nums},  val confident level: {val_confident_level}"
            )
            test_confident_level = (
                predict_prob[test_enhance_nid].argmax(1) == labels[test_enhance_nid]
            ).sum() / len(test_enhance_nid)
            print(
                f"\t\ttest confident nodes: {len(test_enhance_nid)} / {test_node_nums}, test confident_level: {test_confident_level}"
            )

            del train_loader
            train_batch_size = int(
                args.batch_size * len(train_nid) / (len(enhance_nid) + len(train_nid))
            )
            train_loader = torch.utils.data.DataLoader(
                train_idx,
                sampler=sampler,
            ...
            enhance_batch_size = int(
                args.batch_size * len(enhance_nid) / (len(enhance_nid) + len(train_nid))
            )
        ...
        if args.label_feats:
            ...
            label_onehot[train_nid] = F.one_hot(
                init_labels[train_nid], n_classes
            ).float()
            ...
            if args.dataset in ["ogbn-proteins", "ogbn-products"]:  # homogeneous
                ...
                for hop in range(args.num_label_hops):
                    g.update_all(
                        fn.copy_u("m" * hop + "s", "msg"),
                        fn.mean("msg", "m" * (hop + 1) + "s"),
                    )
                ...
                mm_diag, mmm_diag = torch.load(f"./data/{args.dataset}_diag.pt")
                diag_values = {"mm": mm_diag, "mmm": mmm_diag}
                for k in diag_values:
                    if k in label_feats:
                        label_feats[k] = (
                            label_feats[k] - diag_values[k].unsqueeze(-1) * label_onehot
                        )
                ...
                check_acc(
                    label_feats, condition, init_labels, train_nid, val_nid, test_nid
                )
                ...
                # remove self effect on label feats
                mm_diag, mt_diag, tm_diag, tt_diag = torch.load(
                    f"./data/{args.dataset}_diag.pt"
                )
                diag_values = {
                    "mm": mm_diag,
                    "mt": mt_diag,
                ...
                extra_metapath = [
                    ele for ele in extra_metapath if len(ele) > args.num_label_hops + 1
                ]

                print(f"Current num label hops = {args.num_label_hops}")
                if len(extra_metapath):
                    max_hops = max(
                        args.num_label_hops + 1,
                        max([len(ele) for ele in extra_metapath]),
                    )
                else:
                ...
                for k in ["PPP", "PAP", "PFP", "PPPP", "PAPP", "PPAP", "PFPP", "PPFP"]:
                    if k in label_feats:
                        ...
                        label_feats[k] = (
                            label_feats[k] - diag.unsqueeze(-1) * label_onehot
                        )
                        assert torch.all(label_feats[k] > -1e-6)
                        print(k, torch.sum(label_feats[k] < 0), label_feats[k].min())
        ...
        for batch_idx in range((num_nodes - trainval_point - 1) // args.batch_size + 1):
            ...
            batch_end = min(
                num_nodes, (batch_idx + 1) * args.batch_size + trainval_point
            )

            batch_feats = {k: v[batch_start:batch_end] for k, v in feats.items()}
            batch_label_feats = {
                k: v[batch_start:batch_end] for k, v in label_feats.items()
            }
            ...
            if epoch % args.eval_every == 0:
                with torch.no_grad():
                    ...
                    for batch_feats, batch_label_feats, batch_labels_emb in eval_loader:
                        ...
                        batch_label_feats = {
                            k: v.to(device) for k, v in batch_label_feats.items()
                        }
                    ...
                    loss_val = loss_fcn(
                        raw_preds[:valid_node_nums],
                        labels[trainval_point:valtest_point],
                    ).item()
                    loss_test = loss_fcn(
                        raw_preds[valid_node_nums : valid_node_nums + test_node_nums],
                        labels[valtest_point:total_num_nodes],
                    ).item()

                    preds = raw_preds.argmax(dim=-1)
                    val_acc = evaluator(
                        preds[:valid_node_nums], labels[trainval_point:valtest_point]
                    )
                    test_acc = evaluator(
                        preds[valid_node_nums : valid_node_nums + test_node_nums],
                        labels[valtest_point:total_num_nodes],
                    )
        ...
        # raw_preds = gen_output_torch(
        #     model, feats, label_feats, label_emb, all_loader, device
        # )
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(description="SeHGNN", allow_abbrev=False)
    ...
    parser.add_argument(
        "--extra-embedding", type=str, default="", help="the name of extra embeddings"
    )
    parser.add_argument(
        "--embed-size",
        type=int,
    ...
    parser.add_argument(
        "--input-drop", type=float, default=0.1, help="input dropout of input features"
    )
    parser.add_argument(
        "--att-drop", type=float, default=0.0, help="attention dropout of model"
    )
    parser.add_argument(
        "--label-drop", type=float, default=0.0, help="label feature dropout of model"
    )
    parser.add_argument(
        "--residual",
        action="store_true",
    ...
    parser.add_argument(
        "--act", type=str, default="relu", help="the activation function of the model"
    )
    parser.add_argument(
        "--bns",
        action="store_true",
    ...
    parser.add_argument(
        "--gama", type=float, default=0.5, help="parameter for the KL loss"
    )
