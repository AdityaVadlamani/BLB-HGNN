...
class SparseAdjList(object):
    ...
    def __init__(
        ...
        if self.with_values:
            self.fp_float32 = np.memmap(
                self.filename_float32, dtype=self.value_type, mode="r"
            )
        if self.num_adjs > 1:
    ...
    def write(self, adjs):
        ...
        avaiable_nodes = []
        for adj in adjs:
            avaiable_nodes.append(
                torch.unique_consecutive(adj.storage.row(), return_counts=True)
            )

        buffer_int64_sizes = [  # avaiable_nodes + rowptr + col
            len(nodes[0]) + (len(nodes[0]) + 1) + adj.nnz()
            for nodes, adj in tqdm(zip(avaiable_nodes, adjs))
        ]
        buffer_float32_sizes = [adj.nnz() for adj in adjs]  # value
        ...
        fp_int64 = np.memmap(
            ...
            dtype=np.int64,
            mode="w+",
            shape=(file_int64_total_size),
        )
        if self.with_values:
            fp_float32 = np.memmap(
                self.filename_float32,
                dtype=self.value_type,
                mode="w+",
                shape=(file_float32_total_size),
            )
        ...
        fp_int64[4:offset] = np.stack(
            (adjs_int64_offset, adjs_float32_offset), axis=1
        ).flatten()
        ...
        for i, adj in tqdm(enumerate(adjs)):
            ...
            fp_int64[idx_start + num_nodes + 1 : idx_start + num_nodes + num_rowptr] = (
                np.cumsum(avaiable_nodes[i][1].numpy())
            )
            fp_int64[idx_start + num_nodes + num_rowptr : idx_end] = (
                adj.storage.col().data.numpy()
            )
            if self.with_values:
    ...
    def load_adj(
        self, adj_idx=None, adj_key=None, copy=False, expand=False, highlevel=False
    ):
        ...
        idx_start, val_start, idx_end, val_end = self.fp_int64[
            2 + 2 * adj_idx : 2 + 2 * adj_idx + 4
        ]
        num_col = val_end - val_start
        num_nodes = (idx_end - idx_start - num_col) // 2
        num_rowptr = num_nodes + 1
        ...
        avaiable_nodes = torch.LongTensor(
            self.fp_int64[idx_start : idx_start + num_nodes]
        )
        rowptr = torch.LongTensor(
            self.fp_int64[idx_start + num_nodes : idx_start + num_nodes + num_rowptr]
        )
        col = torch.LongTensor(
            self.fp_int64[idx_start + num_nodes + num_rowptr : idx_end]
        )
        if self.with_values:
        ...
        adj = SparseTensor(
            ...
            col=col,
            value=value,
            sparse_sizes=(len(avaiable_nodes), self.num_col),
            is_sorted=True,
        )
        ...
        if expand:
            ...
            return SparseTensor(
                row=avaiable_nodes[row],
                col=col,
                value=value,
                sparse_sizes=(self.num_row, self.num_col),
                is_sorted=True,
            )
        else:
            return adj
    ...
    def load_adjs(self, copy=False, expand=False):
        if self.num_adjs == 1:
            ...
            return {
                k: self.load_adj(i, copy=copy, expand=expand)
                for i, k in enumerate(self.keys)
            }
    ...
    def get_rowcounts(self, node_idices, adj_idices):
        ...
        idx_starts, val_starts, idx_ends, val_ends = start_ends[
            2 * adj_idices.reshape((1, -1)) + np.arange(4).reshape((-1, 1))
        ]
        ...
        avaiable_nodes = self.fp_int64[
            np.concatenate(
                [
                    np.arange(idx_start, idx_start + num)
                    for idx_start, num in zip(idx_starts, num_nodes)
                ]
            )
        ]
        ...
        rowptr_offsets = self.fp_int64[
            np.stack((rowptr_offset_indices, rowptr_offset_indices + 1), axis=1)
        ]
    ...
    def get_cols(self, node_idices, adj_idices):
        ...
        col_indices = []
        for (rowptr_start, rowptr_end), idx_start, num, num_rowptr in zip(
            rowptr_offsets, idx_starts, num_nodes, num_rowptrs
        ):
            if rowptr_start >= 0:
                col_indices.append(
                    np.arange(rowptr_start, rowptr_end) + idx_start + num + num_rowptr
                )
    ...
    def load_batch(
        self, nodes, avoid_triplets=None, sample_metapath_ratio=1.0, sample_neighbors=-1
    ):
        ...
        for i in range(self.num_adjs):
            ...
            _, (nodes_offset, avaiable_nodes) = snp.intersect(
                all_nodes, nodes, indices=True
            )
            if len(avaiable_nodes) == 0:
                continue

            if sample_metapath_ratio < 1.0:
            ...
            ptr_slices = np.stack(
                (
                    idx_start + num_nodes + nodes_offset,
                    idx_start + num_nodes + nodes_offset + 1,
                ),
                axis=1,
            )
            ...
            col_slices = np.concatenate(
                [
                    np.arange(
                        idx_start + num_nodes + num_rowptr + ptr_start,
                        idx_start + num_nodes + num_rowptr + ptr_end,
                    )
            ...
            tmp = SparseTensor(
                ...
                col=torch.LongTensor(self.fp_int64[col_slices].copy()),
                value=value,
                sparse_sizes=(len(avaiable_nodes), self.num_col),
                is_sorted=True,
            )
            ...
            if i in _tmp:  # for avoid_triplets
                n0, (n1, n2) = snp.intersect(
                    np.array(list(_tmp[i].keys())), avaiable_nodes, indices=True
                )
                if len(n0):
                    mask = torch.ones(tmp.nnz(), dtype=torch.bool)
                    for k, idx in zip(n2, n0):
                        vs = _tmp[i][idx]
                        k_offset_s, k_offset_e = (
                            tmp.storage._rowptr[k],
                            tmp.storage._rowptr[k + 1],
                        )
                        flag = False
                        for v in vs:
                            v_idx = np.searchsorted(
                                tmp.storage._col[k_offset_s:k_offset_e].numpy(), v
                            )
                        ...
                        if flag:
                            tmp.storage._value[k_offset_s:k_offset_e] /= (
                                tmp.storage._value[k_offset_s:k_offset_e].sum() + 1e-12
                            )
            ...
            if sample_neighbors > 0 and tmp.storage.rowcount().max() > sample_neighbors:
                tmp_adj, tmp_nid = tmp.sample_adj(
                    torch.arange(tmp.size(0)), sample_neighbors
                )
                tmp = SparseTensor(
                    rowptr=tmp_adj.storage.rowptr(),
                    col=tmp_nid[tmp_adj.storage.col()],
                    value=tmp_adj.storage.value(),
                    sparse_sizes=tmp.sizes(),
                )
                if tmp.storage.value() is not None:
                    tmp.storage._value = (
                        tmp.storage.value() / tmp.sum(dim=1)[tmp.storage.row()]
                    )
