...
class BipartiteEmbedder(nn.Module):
    def __init__(
        ...
        self.m2v_fnn = MaskedFNN(m2v_feat_dim, embed_dim)
        self.m2v_neigh_fnn = MaskedFNN(m2v_feat_dim, embed_dim)
    ...
    def forward(
        ...
        m2v_neighbors_features: torch.Tensor,
    ) -> torch.Tensor:
        transformer_input = torch.concat(
            (
                self.m2v_fnn(m2v_features).unsqueeze(1)
                + self.node_fnn(node_features.unsqueeze(1)),
                (
                    self.m2v_neigh_fnn(m2v_neighbors_features)
                    + self.neighbor_fnn(neighbors_features)
                )
                * neighbors_weights,
            ),
            dim=1,
