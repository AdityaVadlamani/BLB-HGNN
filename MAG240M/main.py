...
from mag240m_data_module import MAG240MDataModule
...
def parse_args():
    ...
    parser.add_argument(
        "--sampler_type",
        default="uniform",
        choices=["uniform", "ppr", "ppr_weighted", "ss", "ppr_ss", "ppr_ss_weighted"],
        help="sampler type",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use validation set as test set and construct validation/hold-out set from training",
    )
...
def main() -> None:
    ...
    datamodule = MAG240MDataModule(
        ...
        construct_new_val_set=args.construct_new_val_set,
    )
    ...
    if not args.skip_replicas:
        for i in range(args.start_replica, args.end_replica + 1):
            ...
            trainer.test(model=model, datamodule=datamodule, ckpt_path=resume_ckpt_path)
            wandb.finish()
    ...
    if trainer is not None:
        ...
        test_acc = ensembled_model.inference_test(datamodule.test_dataloader())
