...
def parse_args():
    ...
    parser.add_argument(
        "--dataset",
        default="IMDB",
        choices=["IMDB", "OGB_MAG"],
        help="torch_geometric dataset",
    )
...
def main():
    ...
    if args.dataset == "OGB_MAG":
        dataset = OGB_MAG(root=os.path.join(DATASET_DIRECTORY, args.dataset))[0]
        num_papers = len(dataset["paper"].y)
        num_authors = dataset["author"].num_nodes
        num_inst = dataset["institution"].num_nodes
        num_fos = dataset["field_of_study"].num_nodes
        ...
        if not os.path.exists(full_edge_list_path):
            pp = dataset.get_edge_store("paper", "cites", "paper")["edge_index"].numpy()
            pa = dataset.get_edge_store("author", "writes", "paper")[
                "edge_index"
            ].numpy() + np.array([[num_papers], [0]])
            ai = dataset.get_edge_store("author", "affiliated_with", "institution")[
                "edge_index"
            ].numpy() + np.array([[num_papers], [num_papers + num_authors]])
            pf = dataset.get_edge_store("paper", "has_topic", "field_of_study")[
                "edge_index"
            ].numpy() + np.array([[0], [num_papers + num_authors + num_inst]])

            full = np.concatenate(
                (pp, pa, ai, pf), axis=1, dtype=np.uint32, casting="unsafe"
            ).T
            np.savetxt("temp.csv", full, fmt="%u", delimiter=",")
            g = gt.load_graph_from_csv("temp.csv", hashed=False)
            g.save(full_edge_list_path)

        num_nodes = num_papers + num_authors + num_inst + num_fos
        training_nodes = dataset["paper"].train_mask.nonzero().squeeze().numpy()
    else:
        dataset = IMDB(root=os.path.join(DATASET_DIRECTORY, args.dataset))[0]
        num_movies = dataset["movie"].x.size(0)
        num_actors = dataset["actor"].x.size(0)
        num_directors = dataset["director"].x.size(0)

        full_edge_list_path = os.path.join(
            DATASET_DIRECTORY, args.dataset, "full_edge_list.gt"
        )
        if not os.path.exists(full_edge_list_path):
            ma = dataset.get_edge_store("movie", "to", "actor")[
                "edge_index"
            ].numpy() + np.array([[0], [num_movies]])
            md = dataset.get_edge_store("movie", "to", "director")[
                "edge_index"
            ].numpy() + np.array([[0], [num_movies + num_actors]])

            full = np.concatenate((ma, md), axis=1, dtype=np.uint32, casting="unsafe").T
            np.savetxt("temp.csv", full, fmt="%u", delimiter=",")
            g = gt.load_graph_from_csv("temp.csv", hashed=False)
            g.save(full_edge_list_path)

        num_nodes = num_movies + num_actors + num_directors
        training_nodes = dataset["movie"].train_mask.nonzero().squeeze().numpy()
    if os.path.exists("temp.csv"):
        os.remove("temp.csv")

    gt.openmp_set_num_threads(args.ncpus)
    ...
    if args.method == "ppr":
        ...
        np.save(
            os.path.join(
                DATASET_DIRECTORY,
                args.dataset,
                f"spread_sampled_p={args.infection_rate}_k={args.removal_threshold}_b={args.target_frac}.npy",
            ),
            S.astype(int),
        )
        print("Saved results to disk")
